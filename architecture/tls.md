...
**T13d (in-process `isActive` at 1.3 with record protection):** offering `TLS_V13` (and a policy that accepts it) writes **legacy 0x0303**, `supported_versions` [1.3, 1.2], x25519 `key_share` (0x001d), and suites 0x1301/02/03 first. After ServerHello both sides install handshake traffic keys on `TLS13RecordLayer`. EncryptedExtensions, Certificate, CertificateVerify, and Finished are AEAD-wrapped as outer `application_data` (RFC 8446 5.2). After the server Finished is in the transcript, application traffic secrets (`c ap` / `s ap`) replace the handshake keys **one direction at a time** (`TLS13CipherState.deriveWriteTrafficKey` / `deriveReadTrafficKey`, matching C++ `derive_{write,read}_traffic_key`): the server switches write to `s ap` after sending Finished and read to `c ap` after the client's Finished; the client switches read to `s ap` before sending Finished and write to `c ap` after. Replacing the whole `Unique!TLS13CipherState` would reset the other direction's sequence (and, on Unique.opAssign, risk dropping the third CS). Servers accept an unprotected Alert until the client's Finished (`mustExpectUnprotectedAlert`, C++ `must_expect_unprotected_alert_traffic`). Default `TLSPolicy` still rejects 1.3; `latestTlsVersion()` stays 1.2. `TestPolicy` accepts 1.3 so `tls/test.d` runs an in-process 1.3 handshake.
