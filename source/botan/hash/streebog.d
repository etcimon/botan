...
/**
* Streebog-256 / Streebog-512. SCAN: "Streebog-256", "Streebog-512".
*/
...
public:
    this(size_t output_bits = 512)
    {
        if (output_bits != 256 && output_bits != 512)
            throw new InvalidArgument("Streebog: Invalid output length");
        m_output_bits = output_bits;
        m_buffer = SecureVector!ubyte(64);
        m_h = SecureVector!ulong(8);
        m_S = SecureVector!ulong(8);
        clear();
    }

    override @property size_t hashBlockSize() const { return 64; }
    override @property size_t outputLength() const { return m_output_bits / 8; }
    override HashFunction clone() const { return new Streebog(m_output_bits); }
    override @property string name() const { return "Streebog-" ~ to!string(m_output_bits); }
    ...
    override void clear()
    {
        ...
        zeroise(m_buffer);
        zeroise(m_S);
        const ulong fill = (m_output_bits == 512) ? 0 : 0x0101010101010101UL;
        foreach (i; 0 .. 8)
            m_h[i] = fill;
    }
...
protected:
    override void addData(const(ubyte)* input, size_t length)
    {
        ...
        if (m_bufpos)
        {
            const size_t take = min(64 - m_bufpos, length);
    ...
    {
        ensureTables();
        const size_t pos = m_bufpos;
        m_buffer[m_bufpos] = 0x01;
        if (m_bufpos + 1 < 64)
            clearMem(&m_buffer[m_bufpos + 1], 64 - m_bufpos - 1);
        compress(m_buffer.ptr, false);
        m_count += pos * 8;

        zeroise(m_buffer);
        storeLittleEndian(m_count, m_buffer.ptr);
        compress(m_buffer.ptr, true);

        compress64(m_S.ptr, true);

        const size_t offset = 8 - outputLength() / 8;
        foreach (i; 0 .. outputLength() / 8)
            storeLittleEndian(m_h[offset + i], output + 8 * i);

        clear();
    }

private:
    void compress(const(ubyte)* input, bool last_block)
    {
        ulong[8] M;
        foreach (i; 0 .. 8)
            M[i] = loadLittleEndian!ulong(input, i);
        compress64(M.ptr, last_block);
    }

    void compress64(const(ulong)* M, bool last_block)
    {
        const ulong N = last_block ? 0 : forceLe(m_count);
        ...
        incrementS(last_block, M);
    }

    void incrementS(bool last_block, const(ulong)* M)
    {
        if (last_block)
            return;
        ulong carry = 0;
        foreach (i; 0 .. 8)
        {
            const ulong m = forceLe(M[i]);
            const ulong hi = forceLe(m_S[i]);
            const ulong t = hi + m + carry;
            m_S[i] = forceLe(t);
            if (t != m)
                carry = (t < m) ? 1 : 0;
        }
...
ulong forceLe(ulong x)
{
    version (LittleEndian)
        return x;
    else
    {
        import botan.utils.bswap : reverseBytes;
        return reverseBytes(x);
    }
}

__gshared immutable ubyte[256] STREEBOG_S = [
...
__gshared immutable ulong[8] STREEBOG_L = [
    0x641c314b2b8ee083, 0xa48b474f9ef5dc18, 0xf97d86d98a327728, 0x5b068c651810a89e,
    0x0321658cba93c138, 0xaccc9ca9328a8950, 0x46b60f011a83988e, 0x83478b07b2468764
];

__gshared immutable ulong[8][12] STREEBOG_C = [
...
void ensureTables()
{
    ...
    foreach (j; 0 .. 8)
        foreach (x; 0 .. 256)
            Ax[j][x] = polyMul1D(STREEBOG_L[j], STREEBOG_S[x]);
    tables_ready = true;
}

void lps(ref ulong[8] block)
{
    ubyte[64] r;
    foreach (i; 0 .. 8)
        storeLittleEndian(block[i], r.ptr + 8 * i);

    foreach (i; 0 .. 8)
    {
        block[i] = forceLe(Ax[0][r[i + 0 * 8]]) ^ forceLe(Ax[1][r[i + 1 * 8]])
            ^ forceLe(Ax[2][r[i + 2 * 8]]) ^ forceLe(Ax[3][r[i + 3 * 8]])
            ^ forceLe(Ax[4][r[i + 4 * 8]]) ^ forceLe(Ax[5][r[i + 5 * 8]])
            ^ forceLe(Ax[6][r[i + 6 * 8]]) ^ forceLe(Ax[7][r[i + 7 * 8]]);
    }
}
