...
*/
enum string SHA2_64_F(alias _A, alias _B, alias _C, alias _D, alias _E, alias _F, alias _G, alias _H, alias _M1, alias _M2, alias _M3, alias _M4, ulong magic) = q{
    %9$s += %1$sUL + rho(%6$s, 14, 18, 41) + ((%6$s & %7$s) ^ (~%6$s & %8$s)) + %10$s;
    %5$s += %9$s;
    %9$s += rho(%2$s, 28, 34, 39) + ((%2$s & %3$s) | ((%2$s | %3$s) & %4$s));
    %10$s += sigma(%11$s, 19, 61, 6) + %12$s + sigma(%13$s, 1, 8, 7);
}.format(magic,
