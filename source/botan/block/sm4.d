/**
* SM4
*
* Copyright:
* (C) 2017 Ribose Inc
* (C) 2018 Jack Lloyd
* (C) 2014-2026 Etienne Cimon
*
* License:
* Botan is released under the Simplified BSD License (see LICENSE.md)
*/
module botan.block.sm4;

import botan.constants;
static if (BOTAN_HAS_SM4):

import botan.block.block_cipher;
import botan.utils.loadstor;
import botan.utils.rotate;
import botan.utils.get_byte;
import botan.utils.mem_ops;
import botan.utils.types;
import botan.utils.cpuid;
static if (BOTAN_HAS_SM4_HWAES) import botan.block.sm4_hwaes;

/**
* SM4 (GB/T 32907-2016). SCAN: "SM4". 128-bit block and key.
* Key schedule uses a constant-time S-box scan (C++ SM4_Tp).
*/
final class SM4 : BlockCipherFixedParams!(16, 16), BlockCipher, SymmetricAlgorithm
{
public:
    override void encryptN(const(ubyte)* input, ubyte* output, size_t blocks)
    {
        static if (BOTAN_HAS_SM4_HWAES)
        {
            if (CPUID.hasAesNi() && CPUID.hasSsse3())
            {
                sm4HwaesEncrypt(input, output, blocks, m_RK.ptr);
                return;
            }
        }
        sm4PortableEncrypt(input, output, blocks, m_RK);
    }

    override void decryptN(const(ubyte)* input, ubyte* output, size_t blocks)
    {
        static if (BOTAN_HAS_SM4_HWAES)
        {
            if (CPUID.hasAesNi() && CPUID.hasSsse3())
            {
                sm4HwaesDecrypt(input, output, blocks, m_RK.ptr);
                return;
            }
        }
        sm4PortableDecrypt(input, output, blocks, m_RK);
    }

    override void clear() { zap(m_RK); }
    @property string name() const { return "SM4"; }
    override @property size_t parallelism() const
    {
        static if (BOTAN_HAS_SM4_HWAES)
            return 4;
        else
            return 1;
    }
    override BlockCipher clone() const { return new SM4; }
    override size_t blockSize() const { return super.blockSize(); }
    override KeyLengthSpecification keySpec() const { return super.keySpec(); }

protected:
    override void keySchedule(const(ubyte)* key, size_t)
    {
        __gshared immutable uint[4] FK = [0xA3B1BAC6, 0x56AA3350, 0x677D9197, 0xB27022DC];
        __gshared immutable uint[32] CK = [
            0x00070E15, 0x1C232A31, 0x383F464D, 0x545B6269, 0x70777E85, 0x8C939AA1, 0xA8AFB6BD, 0xC4CBD2D9,
            0xE0E7EEF5, 0xFC030A11, 0x181F262D, 0x343B4249, 0x50575E65, 0x6C737A81, 0x888F969D, 0xA4ABB2B9,
            0xC0C7CED5, 0xDCE3EAF1, 0xF8FF060D, 0x141B2229, 0x30373E45, 0x4C535A61, 0x686F767D, 0x848B9299,
            0xA0A7AEB5, 0xBCC3CAD1, 0xD8DFE6ED, 0xF4FB0209, 0x10171E25, 0x2C333A41, 0x484F565D, 0x646B7279
        ];

        uint K0 = loadBigEndian!uint(key, 0) ^ FK[0];
        uint K1 = loadBigEndian!uint(key, 1) ^ FK[1];
        uint K2 = loadBigEndian!uint(key, 2) ^ FK[2];
        uint K3 = loadBigEndian!uint(key, 3) ^ FK[3];

        m_RK = SecureVector!uint(32);
        for (size_t i = 0; i != 32; i += 4)
        {
            K0 ^= sm4Tp(K1 ^ K2 ^ K3 ^ CK[i + 0]);
            K1 ^= sm4Tp(K0 ^ K2 ^ K3 ^ CK[i + 1]);
            K2 ^= sm4Tp(K0 ^ K1 ^ K3 ^ CK[i + 2]);
            K3 ^= sm4Tp(K0 ^ K1 ^ K2 ^ CK[i + 3]);
            m_RK[i + 0] = K0;
            m_RK[i + 1] = K1;
            m_RK[i + 2] = K2;
            m_RK[i + 3] = K3;
        }
    }

    package ref const(SecureVector!uint) roundKeys() const { return m_RK; }

private:
    SecureVector!uint m_RK;
}

private:

__gshared immutable ubyte[256] SM4_SBOX = [
    
];

__gshared immutable uint[256] SM4_SBOX_T = [
    0x8ED55B5B, 0xD0924242, 0x4DEAA7A7, 0x06FDFBFB, 0xFCCF3333, 0x65E28787, 0xC93DF4F4, 0x6BB5DEDE,
    0x4E165858, 0x6EB4DADA, 0x44145050, 0xCAC10B0B, 0x8828A0A0, 0x17F8EFEF, 0x9C2CB0B0, 0x11051414,
    0x872BACAC, 0xFB669D9D, 0xF2986A6A, 0xAE77D9D9, 0x822AA8A8, 0x46BCFAFA, 0x14041010, 0xCFC00F0F,
    0x02A8AAAA, 0x54451111, 0x5F134C4C, 0xBE269898, 0x6D482525, 0x9E841A1A, 0x1E061818, 0xFD9B6666,
    0xEC9E7272, 0x4A430909, 0x10514141, 0x24F7D3D3, 0xD5934646, 0x53ECBFBF, 0xF89A6262, 0x927BE9E9,
    0xFF33CCCC, 0x04555151, 0x270B2C2C, 0x4F420D0D, 0x59EEB7B7, 0xF3CC3F3F, 0x1CAEB2B2, 0xEA638989,
    0x74E79393, 0x7FB1CECE, 0x6C1C7070, 0x0DABA6A6, 0xEDCA2727, 0x28082020, 0x48EBA3A3, 0xC1975656,
    0x80820202, 0xA3DC7F7F, 0xC4965252, 0x12F9EBEB, 0xA174D5D5, 0xB38D3E3E, 0xC33FFCFC, 0x3EA49A9A,
    0x5B461D1D, 0x1B071C1C, 0x3BA59E9E, 0x0CFFF3F3, 0x3FF0CFCF, 0xBF72CDCD, 0x4B175C5C, 0x52B8EAEA,
    0x8F810E0E, 0x3D586565, 0xCC3CF0F0, 0x7D196464, 0x7EE59B9B, 0x91871616, 0x734E3D3D, 0x08AAA2A2,
    0xC869A1A1, 0xC76AADAD, 0x85830606, 0x7AB0CACA, 0xB570C5C5, 0xF4659191, 0xB2D96B6B, 0xA7892E2E,
    0x18FBE3E3, 0x47E8AFAF, 0x330F3C3C, 0x674A2D2D, 0xB071C1C1, 0x0E575959, 0xE99F7676, 0xE135D4D4,
    0x661E7878, 0xB4249090, 0x360E3838, 0x265F7979, 0xEF628D8D, 0x38596161, 0x95D24747, 0x2AA08A8A,
    0xB1259494, 0xAA228888, 0x8C7DF1F1, 0xD73BECEC, 0x05010404, 0xA5218484, 0x9879E1E1, 0x9B851E1E,
    0x84D75353, 0x00000000, 0x5E471919, 0x0B565D5D, 0xE39D7E7E, 0x9FD04F4F, 0xBB279C9C, 0x1A534949,
    0x7C4D3131, 0xEE36D8D8, 0x0A020808, 0x7BE49F9F, 0x20A28282, 0xD4C71313, 0xE8CB2323, 0xE69C7A7A,
    0x42E9ABAB, 0x43BDFEFE, 0xA2882A2A, 0x9AD14B4B, 0x40410101, 0xDBC41F1F, 0xD838E0E0, 0x61B7D6D6,
    0x2FA18E8E, 0x2BF4DFDF, 0x3AF1CBCB, 0xF6CD3B3B, 0x1DFAE7E7, 0xE5608585, 0x41155454, 0x25A38686,
    0x60E38383, 0x16ACBABA, 0x295C7575, 0x34A69292, 0xF7996E6E, 0xE434D0D0, 0x721A6868, 0x01545555,
    0x19AFB6B6, 0xDF914E4E, 0xFA32C8C8, 0xF030C0C0, 0x21F6D7D7, 0xBC8E3232, 0x75B3C6C6, 0x6FE08F8F,
    0x691D7474, 0x2EF5DBDB, 0x6AE18B8B, 0x962EB8B8, 0x8A800A0A, 0xFE679999, 0xE2C92B2B, 0xE0618181,
    0xC0C30303, 0x8D29A4A4, 0xAF238C8C, 0x07A9AEAE, 0x390D3434, 0x1F524D4D, 0x764F3939, 0xD36EBDBD,
    0x81D65757, 0xB7D86F6F, 0xEB37DCDC, 0x51441515, 0xA6DD7B7B, 0x09FEF7F7, 0xB68C3A3A, 0x932FBCBC,
    0x0F030C0C, 0x03FCFFFF, 0xC26BA9A9, 0xBA73C9C9, 0xD96CB5B5, 0xDC6DB1B1, 0x375A6D6D, 0x15504545,
    0xB98F3636, 0x771B6C6C, 0x13ADBEBE, 0xDA904A4A, 0x57B9EEEE, 0xA9DE7777, 0x4CBEF2F2, 0x837EFDFD,
    0x55114444, 0xBDDA6767, 0x2C5D7171, 0x45400505, 0x631F7C7C, 0x50104040, 0x325B6969, 0xB8DB6363,
    0x220A2828, 0xC5C20707, 0xF531C4C4, 0xA88A2222, 0x31A79696, 0xF9CE3737, 0x977AEDED, 0x49BFF6F6,
    0x992DB4B4, 0xA475D1D1, 0x90D34343, 0x5A124848, 0x58BAE2E2, 0x71E69797, 0x64B6D2D2, 0x70B2C2C2,
    0xAD8B2626, 0xCD68A5A5, 0xCB955E5E, 0x624B2929, 0x3C0C3030, 0xCE945A5A, 0xAB76DDDD, 0x867FF9F9,
    0xF1649595, 0x5DBBE6E6, 0x35F2C7C7, 0x2D092424, 0xD1C61717, 0xD66FB9B9, 0xDEC51B1B, 0x94861212,
    0x78186060, 0x30F3C3C3, 0x897CF5F5, 0x5CEFB3B3, 0xD23AE8E8, 0xACDF7373, 0x794C3535, 0xA0208080,
    0x9D78E5E5, 0x56EDBBBB, 0x235E7D7D, 0xC63EF8F8, 0x8BD45F5F, 0xE7C82F2F, 0xDD39E4E4, 0x68492121
];

ubyte ctEqMask(ubyte a, ubyte b)
{
    const uint x = a ^ b;
    return cast(ubyte)(0u - (x == 0));
}

uint sm4Tp(uint b)
{
    const ubyte b0 = get_byte(0, b);
    const ubyte b1 = get_byte(1, b);
    const ubyte b2 = get_byte(2, b);
    const ubyte b3 = get_byte(3, b);
    ubyte t0 = 0, t1 = 0, t2 = 0, t3 = 0;
    foreach (size_t i; 0 .. 256)
    {
        const ubyte i8 = cast(ubyte) i;
        const ubyte s = SM4_SBOX[i];
        t0 |= s & ctEqMask(i8, b0);
        t1 |= s & ctEqMask(i8, b1);
        t2 |= s & ctEqMask(i8, b2);
        t3 |= s & ctEqMask(i8, b3);
    }
    const uint t = make_uint(t0, t1, t2, t3);
    return t ^ rotateLeft(t, 13) ^ rotateLeft(t, 23);
}

uint sm4Tslow(uint b)
{
    const uint t = make_uint(SM4_SBOX[get_byte(0, b)], SM4_SBOX[get_byte(1, b)],
                             SM4_SBOX[get_byte(2, b)], SM4_SBOX[get_byte(3, b)]);
    return t ^ rotateLeft(t, 2) ^ rotateLeft(t, 10) ^ rotateLeft(t, 18) ^ rotateLeft(t, 24);
}

uint sm4T(uint b)
{
    const uint s0 = SM4_SBOX_T[get_byte(0, b)];
    const uint s1 = SM4_SBOX_T[get_byte(1, b)];
    const uint s2 = SM4_SBOX_T[get_byte(2, b)];
    const uint s3 = SM4_SBOX_T[get_byte(3, b)];
    return s0 ^ rotateRight(s1, 8) ^ rotateRight(s2, 16) ^ rotateRight(s3, 24);
}

package void sm4PortableEncrypt(const(ubyte)* input, ubyte* output, size_t blocks, const ref SecureVector!uint RK)
{
    foreach (size_t i; 0 .. blocks)
    {
        uint B0 = loadBigEndian!uint(input, 0);
        uint B1 = loadBigEndian!uint(input, 1);
        uint B2 = loadBigEndian!uint(input, 2);
        uint B3 = loadBigEndian!uint(input, 3);
        sm4E(B0, B1, B2, B3, RK, 0, true);
        sm4E(B0, B1, B2, B3, RK, 1, false);
        sm4E(B0, B1, B2, B3, RK, 2, false);
        sm4E(B0, B1, B2, B3, RK, 3, false);
        sm4E(B0, B1, B2, B3, RK, 4, false);
        sm4E(B0, B1, B2, B3, RK, 5, false);
        sm4E(B0, B1, B2, B3, RK, 6, false);
        sm4E(B0, B1, B2, B3, RK, 7, true);
        storeBigEndian(output, B3, B2, B1, B0);
        input += 16;
        output += 16;
    }
}

package void sm4PortableDecrypt(const(ubyte)* input, ubyte* output, size_t blocks, const ref SecureVector!uint RK)
{
    foreach (size_t i; 0 .. blocks)
    {
        uint B0 = loadBigEndian!uint(input, 0);
        uint B1 = loadBigEndian!uint(input, 1);
        uint B2 = loadBigEndian!uint(input, 2);
        uint B3 = loadBigEndian!uint(input, 3);
        sm4D(B0, B1, B2, B3, RK, 7, true);
        sm4D(B0, B1, B2, B3, RK, 6, false);
        sm4D(B0, B1, B2, B3, RK, 5, false);
        sm4D(B0, B1, B2, B3, RK, 4, false);
        sm4D(B0, B1, B2, B3, RK, 3, false);
        sm4D(B0, B1, B2, B3, RK, 2, false);
        sm4D(B0, B1, B2, B3, RK, 1, false);
        sm4D(B0, B1, B2, B3, RK, 0, true);
        storeBigEndian(output, B3, B2, B1, B0);
        input += 16;
        output += 16;
    }
}

void sm4E(ref uint B0, ref uint B1, ref uint B2, ref uint B3, const ref SecureVector!uint RK, size_t R, bool slow)
{
    if (slow)
    {
        B0 ^= sm4Tslow(B1 ^ B2 ^ B3 ^ RK[4 * R + 0]);
        B1 ^= sm4Tslow(B2 ^ B3 ^ B0 ^ RK[4 * R + 1]);
        B2 ^= sm4Tslow(B3 ^ B0 ^ B1 ^ RK[4 * R + 2]);
        B3 ^= sm4Tslow(B0 ^ B1 ^ B2 ^ RK[4 * R + 3]);
    }
    else
    {
        B0 ^= sm4T(B1 ^ B2 ^ B3 ^ RK[4 * R + 0]);
        B1 ^= sm4T(B2 ^ B3 ^ B0 ^ RK[4 * R + 1]);
        B2 ^= sm4T(B3 ^ B0 ^ B1 ^ RK[4 * R + 2]);
        B3 ^= sm4T(B0 ^ B1 ^ B2 ^ RK[4 * R + 3]);
    }
}

void sm4D(ref uint B0, ref uint B1, ref uint B2, ref uint B3, const ref SecureVector!uint RK, size_t R, bool slow)
{
    if (slow)
    {
        B0 ^= sm4Tslow(B1 ^ B2 ^ B3 ^ RK[4 * R + 3]);
        B1 ^= sm4Tslow(B2 ^ B3 ^ B0 ^ RK[4 * R + 2]);
        B2 ^= sm4Tslow(B3 ^ B0 ^ B1 ^ RK[4 * R + 1]);
        B3 ^= sm4Tslow(B0 ^ B1 ^ B2 ^ RK[4 * R + 0]);
    }
    else
    {
        B0 ^= sm4T(B1 ^ B2 ^ B3 ^ RK[4 * R + 3]);
        B1 ^= sm4T(B2 ^ B3 ^ B0 ^ RK[4 * R + 2]);
        B2 ^= sm4T(B3 ^ B0 ^ B1 ^ RK[4 * R + 1]);
        B3 ^= sm4T(B0 ^ B1 ^ B2 ^ RK[4 * R + 0]);
    }
}
